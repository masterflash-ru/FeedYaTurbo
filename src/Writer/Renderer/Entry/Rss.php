...
namespace Mf\FeedYaNews\Writer\Renderer\Entry;
...
use Mf\FeedYaNews\Writer\Renderer;
...
class Rss extends Renderer\AbstractRenderer implements Renderer\RendererInterface
{
    ...
    public function render()
    {
        ...
        $this->_setPdaLink($this->dom, $entry);
        $this->_setAmpLink($this->dom, $entry);
        $this->_setId($this->dom, $entry);
        $this->_setGenre($this->dom, $entry);
    ...
    /*рендеринг Genre*/
    protected function _setGenre(DOMDocument $dom, DOMElement $root)
    {
        $genre=$this->getDataContainer()->getGenre();
        if (empty($genre)){
            return;
        }
        $title = $dom->createElement('genre');
        $root->appendChild($title);
